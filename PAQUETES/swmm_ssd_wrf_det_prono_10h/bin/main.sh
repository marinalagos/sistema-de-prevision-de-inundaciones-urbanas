...
find PAQUETES/swmm_ssd_wrf_det_prono_10h/bin -type f -name "run_swmm_*.sh" -exec rm {} +
...
#!/bin/bash
#SBATCH --job-name=wrfpreprocesamiento
#SBATCH --cpus-per-task=1
#SBATCH --ntasks=1
#SBATCH --output=LOGS/wrfpre_%j_$inicio_sim_compacto.log
...
python PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/preprocesamiento.py --inicio_sim "$inicio_sim"
EOF
)
...
#!/bin/bash
#SBATCH --job-name=wrfejecucion_modelo_$inicio_sim_compacto
#SBATCH --output=LOGS/wrfejecucion_modelo_%j_$inicio_sim_compacto.log
#SBATCH --time=00:120:00
#SBATCH --constraint="128667MB"                               
...
if [ -f "PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/run_swmm_$inicio_sim_compacto.sh" ]; then
...
chmod 775 PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/run_swmm_$inicio_sim_compacto.sh

echo "Verificando permisos de ejecución del archivo run_swmm_$inicio_sim_compacto.sh..."
ls -l PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/run_swmm_$inicio_sim_compacto.sh
...
srun PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/run_swmm_$inicio_sim_compacto.sh
...
#!/bin/bash
#SBATCH --job-name=wrfpostprocesamiento
#SBATCH --output=LOGS/wrfpost_%j_$inicio_sim_compacto.log
#SBATCH --time=00:120:00
#SBATCH --constraint="128667MB"                               
#SBATCH --cpus-per-task=1
#SBATCH --ntasks=1
...
echo "$inicio_sim"
python PAQUETES/swmm_ssd_wrf_det_prono_10h/bin/postprocesamiento.py --inicio_sim "$inicio_sim"
EOF
)
