...
def consultar_base_ina(
    ...
    df_pp = pd.concat(series, axis=1)
    df_pp = df_pp.resample(f"{params['dt_precipitacion_minutos']}min", origin='end').mean()
    ...
    df_pp.index = df_pp.index.shift(-1)
    ...
    df_coords = pd.DataFrame(index=ids, data={"lat": lats, "lon": lons})
